...
import { ChevronDown, MoreHorizontal, X, ChevronRight } from 'lucide-react';
...
const NotificationHeader = ({
  ...
  <div
    className={`grid  items-center gap-4 ${isDocuments ? 'pl-8 grid-cols-[1fr_200px_200px_150px_100px]' : 'px-1 grid-cols-[1fr_220px_150px_100px]'} pb-1 pt-3`}
  >
...
const DocumentNotificationItem = ({
  ...
  const getFrequencyLabel = () => {
    ...
    return `${frequencies.length} options selected`;
  };

  return (
    <div className="grid grid-cols-[1fr_200px_200px_150px_100px] gap-4 items-center py-2">
      <div className="font-medium">{category}</div>
      <Select
        value={notification.documentScope || 'all'}
        onValueChange={() => onScopeChange('added', true)}
      >
        ...
        <SelectContent>
          ...
          <SelectItem value="myProjects">Contractors on my projects</SelectItem>
      ...
      <div>
        <Select
          ...
          onValueChange={(value: NotificationFrequency) => onEmailToggle()}
        >
...
const DocumentNotifications = ({
  ...
  const [expandedRemoved, setExpandedRemoved] = useState(false);
      ...
      <div>
        ...
        <AnimatePresence>
          {expandedAdded && (
            ...
            >
              <div
                className={`grid grid-cols-[1fr_200px_150px_100px] items-center gap-4 pl-8 pb-1 pt-3`}
              >
              ...
              <div className="pl-8">
                {documentCategories.map((category) => (
                  <motion.div
                    key={category}
                    className="grid grid-cols-[1fr_200px_150px_100px] gap-4 items-center py-2"
                  ...
                  >
                    <div className="font-medium">{category}</div>
                    <Select
                      value={notifications.newDocuments.documentScope || 'all'}
                    >
                      <SelectTrigger className="w-full">
                        <SelectValue placeholder="Select scope" />
                      </SelectTrigger>
...
export default function NotificationsPage() {
  ...
  const renderUserNotificationItem = (category: string, user: string) => {
    ...
    return (
      <div
        key={user}
        className="grid grid-cols-[1fr_auto_150px_100px] gap-4 items-center"
      >
    ...
    return (
      <div className="flex flex-col gap-y-2 px-1 py-2">
        {category === 'documents' && setting === 'documentExpirations' ? (
          ...
          <div className="grid grid-cols-[1fr_auto_150px_100px] items-center gap-4">
